    ...
    static class LocalDB
    {
        ...
        public static SqlConnection GetLocalDB(string dbName, bool deleteIfExists = false)
        {
            try
            {
                ...
                string outputFolder = Path.Combine(assemblyPath, DB_DIRECTORY);//Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), DB_DIRECTORY);
